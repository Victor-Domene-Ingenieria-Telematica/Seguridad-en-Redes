...
// Digramas comunes en inglés
char *digrams[] = {
    "th", "he", "in", "en", "nt", "re", "er", "an", "ti", "es",
    "on", "at", "se", "nd", "or", "ar", "al", "te", "co", "de",
    "to", "ra", "et", "ed", "it", "sa", "em", "ro", NULL
};

// Trigramas comunes en inglés
char *trigrams[] = {
    "the", "and", "tha", "ent", "ing", "ion", "tio", "for", "nde",
    "has", "nce", "edt", "tis", "oft", "sth", "men", NULL
};

// Struct para cada candidato
struct Candidate {
	int key;
	double distance;
	int cand_digs;
...
// Descifrar con una clave
char*
desencrypt_chars(char *cipher, int len, int key)
{
    char *plain = malloc(len + 1);  // Reservamos uno mas para el \0
    if(plain == NULL)
...
get_trigs(char *text, int len)
{
    ...
    for(i = 0; i < len - 2; i++) {
        char t[4];
        snprintf(t, 4, "%c%c%c", tolower(text[i]), tolower(text[i+1]), tolower(text[i+2]));
...
// Probar todas las claves y calcular sus indicadores
void
try_keys(Candidate *candidates, char *cipher, int len)
{
    for(int i = 0; i < Nkeys; i++) {
        ...
        // Guardamos la clave en el candidato
        candidates[i].key = key;

        // Guardamos el el texto plano (desencriptado) en el candidato
...
// Imprimir el mejor candidato por trigramas (si es diferente)
void
...
main(int argc, char* argv[])
{
    ...
    // Buscar el mejor por distancia (menor es mejor)
    int best_dist = 0;
    ...
    // Buscar el mejor por digramas (mayor es mejor)
    int best_dig = 0;
    for(int i = 1; i < Nkeys; i++) {
    ...
    // Buscar el mejor por trigramas (mayor es mejor)
    int best_tri = 0;
    for(int i = 1; i < Nkeys; i++) {
